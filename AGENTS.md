...
This repository is a public mathematical research repository. Before making substantive changes, agents must establish both the **historical provenance** of EIG and the **current epistemic boundary**. Do not infer current theorem status from an older research note, and do not rewrite historical claims to make later developments look inevitable.
...
For work that touches a specific theorem, application, or discovery, also read the proof/audit/provenance files named by the relevant top-level document before changing its status.
...
Do not use internal agreement, model agreement, filenames such as `FINAL` or `TERMINAL`, or a passing finite regression as substitutes for a mathematical proof or external priority evidence.

Prefer the strongest correct scoped statement over a broader slogan. A negative theorem, moduli classification, or failure object is a successful EIG result when it identifies the exact boundary.

## No-smuggling and canonicality

When proposing a reconstruction theorem, state explicitly:

- what operational/interaction doctrine is supplied;
- which contexts or continuations are admitted;
- what witness/provenance information is retained;
- what equivalence notion is being used;
- what part is constructed canonically;
- what requires a separate density, nervousness, representability, locality, positivity, or operation-exactness gate;
- what residual fibre or moduli remain when reconstruction fails.

Do not call a construction doctrine-free if the target grammar, arities, maps, witnesses, locality, or equivalent answer-adjacent structure has been supplied in the input.
