...
They are also forced. To generate `06`, some generator `x <= 06` must contain bit `2`; to generate `09`, a distinct generator `y <= 09` must contain bit `3`. To generate `07`, neither `x` nor `y` can supply bit `0` without an illegal extra bit, so the third generator `z` must contain bit `0`. To generate `0b`, the same `z` must contain bit `1`. Hence `z=03`. Then `06` can only be generated by `x`, so `x=06`, and similarly `y=09`.
...
The mode-1 unfolding has nonzero columns `01,02,03`, so its Boolean rank is two with unique minimum basis `{01,02}`. Therefore `beta(T)=(2,3,3)`.
...
The checker uses no SAT/SMT/MIP solver, randomness, floating point, normalization lemma, or Python `assert`. It reconstructs the tensor; exhausts every nonzero support mask on each mode to derive the three mode Boolean ranks and unique minimum bases; checks the four blocking zeros; verifies exact profiles `(2,3,4)` and `(2,4,3)`; and checks a same-shape tensor with the same mode-rank vector `(2,3,3)` for which `(2,3,3)` *is* feasible.
