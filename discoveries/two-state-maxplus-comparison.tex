...
\begin{enumerate}[label=(\roman*)]
\item \emph{propagate}: the successor retains an unbounded gap with magnitude $n'=n+s$ for a fixed integer $s$ (and a fixed successor side), while the height increment is a fixed constant $c$;
...
If the current gap is bounded, the exact successor and height increment are obtained from a finite lookup table. If it is infinite, the only surviving current coordinate determines the successor by another finite lookup. Suppose now that the counter stores a tail magnitude $n>K$. Apply Lemma~\ref{lem:tail}.
