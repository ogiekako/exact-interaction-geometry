...
We show that the answer is no for both ranks.\footnote{The examples were found while experimenting with the \emph{Interaction Reconstruction} research programme, \url{https://github.com/ogiekako/interaction-reconstruction}. The broader programme is discovery provenance only and is not used in either proof; its components carry separate audit statuses in that repository.}
...
A literature search performed on August~30, 2026 located the original Section~6 question but no prior published resolution of this specific source-pair problem. We state this only as a bibliographic search result; it should be refreshed immediately before submission.
...
The programme connection is heuristic, not logical. The relevant search idea was to regard the base graph as a finite factorization atlas and look for source-interface incompatibility that is jointly expensive but pairwise invisible. The explicit examples and proofs above stand on their own.
